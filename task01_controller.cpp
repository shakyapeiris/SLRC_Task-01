...
int main(int argc, char **argv) {
  ...
  //*****************************************************************
    ...
    if (dsValue<1000.0 and dsValue>200.0){
        ...
        if (std::find(edited.begin(), edited.end(), round_2) != edited.end() or std::find(edited.begin(), edited.end(), round_2+3)!= edited.end() or std::find(edited.begin(), edited.end(), round_2-3)!= edited.end() or std::find(edited.begin(), edited.end(), round_2+2)!= edited.end() or std::find(edited.begin(), edited.end(), round_2-2)!= edited.end() or std::find(edited.begin(), edited.end(), round_2+1)!= edited.end() or std::find(edited.begin(), edited.end(), round_2-1)!= edited.end()){
            consist = true;
          }
        else{
